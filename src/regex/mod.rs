...
impl RegexF {
    ...
    pub fn app(a: &Self, b: &Self) -> Self {
        match (a, b) {
            ...
            (RegexF::Star(x), RegexF::Star(y)) if RegexF::partial_le(x, y)==Some(true) => b.clone(),
            (RegexF::Star(x), RegexF::Star(y)) if RegexF::partial_le(y, x)==Some(true) => a.clone(),
...
fn test_regex_repetition_range() {
    assert_eq!(
        re::app(
            re::app(re::dotstar(), re::range(re::character('a'), 1, 3)),
            re::dotstar()
        ),
        re::new("a{1,3}")
    );
