    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        fn to_regex(e: &Expr) -> Result<Regex, String> {
            match e {
                ...
                Expr::Repeat { child, lo, hi, .. } => Ok(Regex::range(to_regex(child)?, *lo, *hi)),
...
impl Regex {
    ...
    pub fn app(a: Regex, b: Regex) -> Regex {
        match (&*a.0, &*b.0) {
            ...
            (RegexF::Range(a, i1, j1), RegexF::Range(b, i2, j2)) if a == b => {
                Regex::range(a.clone(), i1 + i2, j1 + j2)
            }
    ...
    pub fn has_zero_length_assertions(&self) -> bool {
        match *self.0 {
            ...
            RegexF::App(ref a, ref b) => {
                a.has_zero_length_assertions() || b.has_zero_length_assertions()
            }
    ...
    pub fn deriv(&self, c: &char) -> Regex {
        match *self.0 {
            ...
            RegexF::Range(ref a, i, j) => match (i..=j).map(|i| a.repeat(i)).reduce(Regex::alt) {
                Some(r) => r.deriv(c),
                None => Regex::nil(),
            },
...
fn regex_parser_test_repetition_range() {
    assert_eq!(
        Regex::range(Regex::character('a'), 1, 3),
        Regex::new("a{1,3}")
    );
}
